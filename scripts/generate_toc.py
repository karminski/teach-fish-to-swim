...
def get_index_creation_time(index_html_path: Path, root_path: Path) -> float:
    """
    Get the creation time of an index.html file as a Unix timestamp.
    First tries to get the first commit time from git, then falls back to file system time.
    Returns float timestamp for sorting (newer = larger value).
    """
    # Get relative path from root for git command
    try:
        relative_path = index_html_path.relative_to(root_path)
    except ValueError:
        relative_path = index_html_path

    # Try git first (more reliable in CI/CD environments)
    try:
        # Get the first commit time for this index.html file
        ...
        if result.returncode == 0 and result.stdout.strip():
            # Get the last line (oldest commit = first creation)
            lines = result.stdout.strip().split("\n")
            oldest_time = lines[-1].strip()
            if oldest_time:
                return float(oldest_time)
    except Exception as e:
        print(f"Warning: Could not get git time for {index_html_path}: {e}")

    # Fallback to file system time of index.html
    try:
        stat = index_html_path.stat()
        # Use creation time on Windows, or modification time on Unix
        if hasattr(stat, "st_birthtime"):
            return stat.st_birthtime
        else:
            return stat.st_mtime
    except Exception as e:
        print(f"Warning: Could not get file system time for {index_html_path}: {e}")
        return 0.0


def get_article_dirs(root_path: Path) -> list[tuple[str, str, float]]:
    """
    Get all article directories with their titles and creation times.
    Returns list of (folder_name, title, creation_timestamp) tuples sorted by creation time (newest first).
    """
    articles = []
    ...
    for item in root_path.iterdir():
        ...
        if title:
            creation_time = get_index_creation_time(index_html, root_path)
            articles.append((item.name, title, creation_time))
...
def generate_toc_html(articles: list[tuple[str, str, float]]) -> str:
    """Generate the TOC list HTML."""
    items = []

    for idx, (folder_name, title, _) in enumerate(articles, start=1):
...
def main():
    ...
    print(f"Found {len(articles)} articles (sorted by creation time, newest first):")
    for folder, title, creation_timestamp in articles:
        if creation_timestamp > 0:
            time_str = datetime.fromtimestamp(creation_timestamp).strftime("%Y-%m-%d")
        else:
            time_str = "unknown"
        print(f"  - [{time_str}] {folder}: {title}")
